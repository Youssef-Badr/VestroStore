...
//   // ✅ أهم حاجة: Variant ID مش Product ID
//   content_ids: [product.variantId.toString()],

//   content_name: product.name,

//   content_type: "product",

//   value: Number(product.price || selectedVariant?.price || 0),
...
//   contents: [
//     {
//       id: product.variantId.toString(), // ✅ لازم Variant ID
//       quantity: Number(qty || 1),
//       item_price: Number(product.price || selectedVariant?.price || 0)
//     }
//   ]
// });
...
//   // ✅ لازم Variant IDs
//   content_ids: bundle.bundleItems.map(
...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = (product, selectedVariant, qty) => {
    ...
    // 🔥 TRACK FIXED (VARIANT ID ONLY)
    fbqTrack("track", "AddToCart", {
      content_ids: [vId.toString()],
      content_name: product.name,
      content_type: "product",
      value: Number(selectedVariant?.price || product.price || 0),
      currency: "EGP",
  ...
  const addBundleToCart = (bundle, selections, qty = 1) => {
    ...
    // 🔥 TRACK FIXED (ALL VARIANT IDS ONLY)
    fbqTrack("track", "AddToCart", {
    ...
    setCart((prev) =>
      prev.map((item) => {
        ...
        if (itemId === id) {
          ...
          if (Number(qty) > limit) {
            toast.warn(`عذراً، المتاح ${limit} فقط`, {
              toastId: "limit",
            });
            return { ...item, qty: limit };
          }
