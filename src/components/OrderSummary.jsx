...
const CartItem = React.memo(
  ({ item, darkMode, isRTL, removeFromCart, updateQty }) => {
    const itemId = item.isBundle ? item.uniqueId : item.variantId;

    const totalPrice = useMemo(() => {
      return (item.price * item.qty).toLocaleString();
    }, [item.price, item.qty]);
    ...
    const handleMinus = useCallback(() => {
      if (item.qty > 1) updateQty(itemId, item.qty - 1);
    }, [updateQty, itemId, item.qty]);
    ...
    return (
      <div
        className={`flex gap-4 p-4 rounded-[1.8rem] border transition-all duration-300 ${
          darkMode
            ? "bg-[#111] border-white/5 hover:border-red-700"
          ...
          {item.isBundle ? (
            <div className="flex w-full h-full">
              {item.bundleItems?.slice(0, 2).map((bi, i) => (
                <img
                  key={i}
                  src={getImage(bi.image, 220)}
                  loading="lazy"
                  decoding="async"
                  className={`${
                    item.bundleItems.length > 1 ? "w-1/2" : "w-full"
                  } h-full object-cover ${
                    i === 0 && item.bundleItems.length > 1
                      ? isRTL
          ...
          <div className="flex flex-col gap-1.5 mt-1">
            {item.isBundle ? (
              ...
              <div className="flex gap-2 mt-1">
                ...
                <span
                  className={`text-[9px] font-bold uppercase px-2 py-0.5 rounded ${
                    darkMode ? "bg-white/5 text-white" : "bg-black text-white"
                  }`}
                >
                  {item.size}
                </span>
            ...
            >
              <button
                ...
                className={`w-5 h-5 flex items-center justify-center rounded transition-all ${
                  item.qty <= 1
                    ? "opacity-10"
                    : "hover:bg-red-700 hover:text-black dark:text-white"
                }`}
              >
                <Minus size={10} strokeWidth={3} />
              </button>
...
const OrderSummary = ({
  ...
  if (!cart || cart.length === 0) {
    return (
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 mb-6 mt-28">
        <div
          className={`flex flex-col items-center justify-center py-16 px-6 rounded-[2.5rem] border-2 border-dashed transition-all ${
            darkMode
        ...
        >
          ...
          >
            <ShoppingBag
              size={48}
              className={darkMode ? "text-red-700" : "text-slate-300"}
            />
          </div>
          ...
          <p className="text-sm font-bold text-slate-500 dark:text-slate-400 mb-8 max-w-[280px] text-center leading-relaxed">
            {isRTL
              ? "لم يتم العثور على أي منتجات، ابدأ بالتسوق الآن!"
              : "No products found in your cart. Start shopping now!"}
          </p>

          <button
            onClick={goShopping}
            className={`group flex items-center gap-3 px-10 py-4 rounded-2xl font-black uppercase transition-all hover:scale-105 active:scale-95 shadow-xl ${
              darkMode
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 mb-6 mt-24">
      <div
      ...
      >
        ...
        >
          ...
          >
            {cart.map((item) => (
              <CartItem
                key={item.isBundle ? item.uniqueId : item.variantId}
                item={item}
                darkMode={darkMode}
                isRTL={isRTL}
